...
# Filtering for cond-mat.mtrl-sci
# Create a new dictionary to store the filtered articles
cond_filtered_mtrl= {}

# Iterate over each journal class and its articles
for journal_class, articles_info in cond_filtered7.items():
    # Create a list to store article IDs to be removed
    articles_to_remove = []

    # Iterate over each article ID and its attributes
    for article_id, article_info in articles_info.items():
        # Check if the value associated with the article ID is a dictionary
        if isinstance(article_info, dict):
            # Check if the category_one is "cond-mat.mtrl-sci"
            if article_info.get('category_one') != "cond-mat.mtrl-sci":
                # Add the article ID to the list of articles to be removed
                articles_to_remove.append(article_id)
        else:
            # If the value is not a dictionary, it's not an article, so skip it
            continue

    # Remove the articles from the nested dictionary
    for article_id in articles_to_remove:
        del articles_info[article_id]

    # Store the filtered articles in the new dictionary
    cond_filtered_mtrl[journal_class] = articles_info

# Print the updated nested dictionary with only "cond-mat.mtrl-sci" articles
print(cond_filtered_mtrl)

total_entries = sum(len(articles_info) for articles_info in cond_filtered_mtrl.values())
print("Total number of entries in cond_filtered_mtrl:", total_entries)
...
for journal, entries in cond_filtered_mtrl.items():
    # Extract the entry IDs from the entries dictionary and add them to the list
...
for key, inner_dict in cond_filtered_mtrl.items():
    # Extract the keys of the inner dictionary and add them to the set
...
for category in categories: 
    for entry_id, entry in train_entries[category].items():
        abstract_tokens = entry.get('abstract_tokens', [])

        # Unigrams
        for word in abstract_tokens:
            unigram_vocab[category][word] += 1
        for word in set(abstract_tokens):
            n_entries_unigrams[category][word] += 1

        # Bigrams
        abstract_bigrams = [' '.join(abstract_tokens[i:i+2]) for i in range(len(abstract_tokens) - 1)]
        for bigram in abstract_bigrams:
            bigram_vocab[category][bigram] += 1
        for bigram in set(abstract_bigrams):
...
def get_nb_predictions(categories, test_entries, probabilities, prob_class):
    ...
    for category in categories:
        ...
        for entry_id, entry in test_entries[category].items():
            # Extract abstract tokens from the entry
            abstract_tokens = entry.get('abstract_tokens', [])
...
def classification_report(labels, prediction):
    ...
    print("Unigram unsmoothed Macro F1-score: {:.2%}".format(macro_f1))
    print('\n'*2)
...
print("Baseline Based on Random Assignment")
classification_report(labels, baseline_preds)

import random

# Baseline based on prior probabilities
    ...
    for category in categories:
        ...
        for word in common:
            if unigram_vocab[category][word] > 0:
                # If they do exist in the current class category, store their count, i.e., the 1st order model
                probabilities[category][word] = unigram_vocab[category][word] + smoothing_alpha
            else:
                # If not, set their count to be the smoothing parameter (rather than excluding them, as we did for no smoothing) i.e., backoff to 0th order model
...
def naive_bayes_bigrams_additive_smoothing(unigram_vocab, bigram_vocab, categories, alpha):
    ...
    for category in categories:
        ...
        # Repeat the same process for the bigram probabilities but normalized by count and smoothing alpha
...
def get_nb_bigram_additive_smoothing_predictions(categories, test_entries, bigram_probabilities, alpha, prob_class):
    ...
    for category in categories:
        ...
        for entry_id, entry in test_entries[category].items():
            ...
            for wordi in range(len(abstract_tokens)-1):
                bigram = '{} {}'.format(abstract_tokens[wordi], abstract_tokens[wordi+1])
...
# Mutual information Naive Bayes functions
n_total = dict()
for category in categories:
    n_total[category] = len(train_entries[category])

def mutual_information(term, train_entries):
    # Count occurrences of the term (unigram or bigram) in abstract_tokens for each journal-ref
    category_counts = defaultdict(int)
    total_counts = defaultdict(int)

    for journal_ref, entries in train_entries.items():
        for entry_id, entry in entries.items():
            if 'abstract_tokens' in entry:
                abstract_tokens = entry['abstract_tokens']
                category = entry.get('journal-ref', None)
                if category:
                    total_counts[category] += len(abstract_tokens) - (1 if len(term) == 1 else 0)
                    if len(term) == 2:
                        category_counts[(term, category)] += sum(1 for i in range(len(abstract_tokens) - 1) if tuple(abstract_tokens[i:i+2]) == term)
                else:
                        category_counts[(term, category)] += abstract_tokens.count(term)
...
def get_predictions(test_entries, probabilities, prob_class, smoothing_alpha, n):
    ...
    for category in categories:
        ...
        for entry_id, entry in test_entries[category].items():
            ...
            if n == 1:
                token_list = entry.get('abstract_tokens', [])
            elif n == 2:
                token_list = [' '.join(abstract_tokens[i:i+2]) for i in range(len(abstract_tokens) - 1)]
...
top_ns = [200, 500, 1000, 5000, 10000, 15000]
...
# Now, use the best parameters to fit the model on the training data and evaluate it on the test data
...
# After searching all parameter combinations, print the best ones
print("Best bigram parameters: alpha = {}, top_n = {} with a macro-f1 score of {}".format(best_params['alpha'], best_params['top_n'], best_f1_score))
# Now, use the best parameters to fit the model on the training data and evaluate it on the test data
