...
# Create vocabularies
for category in categories:  # Assuming each category corresponds to a unique journal reference
    for entry_id, entry in train_entries[category].items():
        abstract_tokens = entry.get('abstract_tokens', [])  # Assuming 'abstract_tokens' contains the tokenized abstract
...
# Descriptive stats for vocabularies
for category in categories:
    num_unique_unigrams = len(unigram_vocab[category])
    num_unique_bigrams = len(bigram_vocab[category])
    print(f"Category: {category}")
    print(f"Number of unique unigrams: {num_unique_unigrams}")
...
def naive_bayes_unsmoothed(unigram_vocab, categories):
    ...
    for category in categories:
        ...
        # Create a partial copy of our unigram_vocab count dict, selecting only words that occur in both classes
...
def naive_bayes_bigrams_additive_smoothing(unigram_vocab, bigram_vocab, categories, alpha):
    ...
    for category in categories:
        ...
        # Normalize by the count of the first word in the bigram
        # Add alpha to the numerator and add len alpha to the denominator
    ...
    for category in categories:
        ...
        for entry_id, entry in test_entries[category].items():
            ...
            for wordi in range(len(abstract_tokens) - 1):
                ...
                # Check if the bigram exists in the bigram vocabulary for that category
                # If it does, add the log probability of the unigram starting at that index to the score for that category
                # If that also does not exist add an arbitrary small value alpha to the score for that category
                for cat in categories:
    ...
    for category in categories:
        ...
        for entry_id, entry in test_entries[category].items():
            ...
            for token in token_list:
                for cat in prob_class.keys():  # Iterate over keys of prob_class
            ...
            for cat in categories:  # Iterate over keys of prob_class
                scores[cat] += np.log(prob_class[cat])
