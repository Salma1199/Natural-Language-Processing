...
# Filtering for cond-mat.mes-hall
...
for journal_class, articles_info in cond_filtered7.items():
    ...
    for article_id, article_info in articles_info.items():
        ...
        if isinstance(article_info, dict):
            # Check if the category_one is "cond-mat.mes-hall"
            if article_info.get('category_one') != "cond-mat.mes-hall":
    ...
    cond_filtered_mes[journal_class] = articles_info

# Print the updated nested dictionary with only "cond-mat.mes-hall" articles
print(cond_filtered_mes)

total_entries = sum(len(articles_info) for articles_info in cond_filtered_mes.values())
print("Total number of entries in cond_filtered_mes:", total_entries)
...
for journal, entries in cond_filtered_mes.items():
...
for key, inner_dict in cond_filtered_mes.items():
...
# Class predictions based on prior probability
